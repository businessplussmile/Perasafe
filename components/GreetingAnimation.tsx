...
const GreetingAnimation: React.FC<GreetingAnimationProps> = ({ member, onComplete }) => {
  ...
  useEffect(() => {
    ...
    const timers = [
      setTimeout(() => setPhase(1), 500),   // Apparition du titre
      setTimeout(() => setPhase(2), 2000),  // Apparition du message
      setTimeout(() => setPhase(3), 6000),  // Début du fondu de sortie
      setTimeout(() => onComplete(), 7000)  // Fin de l'animation
    ];
    ...
    <div className={`fixed inset-0 z-[300] bg-white flex flex-col items-center justify-center transition-opacity duration-1000 ${phase === 3 ? 'opacity-0' : 'opacity-100'}`}>
      ...
      <div className="relative z-10 text-center px-6 max-w-3xl">
        <div className={`mb-12 transition-all duration-1000 transform ${phase >= 1 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <h1 className="font-serif text-4xl md:text-6xl text-slate-900 italic tracking-tight">
            Honneur et Respect,
          </h1>
      ...
      <div className="absolute bottom-0 left-0 h-1 bg-[#F2AF31]/20 w-full">
        <div className="h-full bg-[#643012] transition-all duration-[7000ms] ease-linear" style={{ width: phase < 3 ? '100%' : '100%' }}></div>
      </div>
    </div>
  );
};
